...
# 수집/적재
def collector(stock, trcode, theme, cnt, total_cnt, skip):
    global engine
    ...
    try:
        ...
        # 데이터가 있을 때만 적재
        if len(df) != 0:
            df.drop(["현재가", "등락기호", "전일대비", "등락율", "누적거래량", "매도호가", "매도잔량", "매수호가", "매수잔량", "기간수익률n"], axis=1, inplace=True)
            df["테마코드"] = theme["code"]
            ...
            print("{}/{} {}-{} 수집/적재 완료".format(cnt, total_cnt, trcode, theme["code"]))
            pkey = "{}".format(theme["code"])
            stock.set_batch_result(trcode, pkey, "SUCCESS", skip)
        else:
            print("{}/{} {}-{} 데이터 없음".format(cnt, total_cnt, trcode, theme["code"]))
            pkey = "{}".format(theme["code"])
            stock.set_batch_result(trcode, pkey, "NO DATA", skip)
    except Exception as ex:
        print("{}/{} {}-{} 수집/적재 실패".format(cnt, total_cnt, trcode, theme["code"]))
        pkey = "{}".format(theme["code"])
...
def main(stock, trcode):
    ...
    theme_list = stock.get_list("theme")  # 테마 list 생성
    total_cnt = len(theme_list)
    ...
    if (exitcode == 0):
        ...
        for theme in theme_list:
            ...
            if skip == 1:
                print("{}/{} {}-{} 건너뛰기".format(cnt, total_cnt, trcode, theme["code"]))
            else:
                collector(stock, trcode, theme, cnt, total_cnt, skip)
                use_api = True
