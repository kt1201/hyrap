...
# 수집/적재
def collector(stock, trcode, market, sort, cnt, total_cnt):
    global engine
    ...
    try:
        ...
        if len(df) != 0:
            ...
            print("{}/{} {}-{}-{} 수집/적재 완료".format(cnt, total_cnt, trcode, market["code"], sort))
            pkey = "{}-{}".format(market["code"], sort)
            stock.set_batch_result(trcode, pkey, "SUCCESS")
        else:
            print("{}/{} {}-{}-{} 데이터 없음".format(cnt, total_cnt, trcode, market["code"], sort))
            pkey = "{}-{}".format(market["code"], sort)
            stock.set_batch_result(trcode, pkey, "NO DATA")
    except Exception as ex:
        print("{}/{} {}-{}-{} 수집/적재 실패".format(cnt, total_cnt, trcode, market["code"], sort))
        pkey = "{}-{}".format(market["code"], sort)
...
def main(stock, trcode):
    ...
    sort_list = ['1', '3']  # 정렬 구분 리스트, 1: 상승률, 2: 하락률
    ...
    if (exitcode == 0):
        ...
        for market in market_list:
            for sort in sort_list:
                cnt = cnt + 1
                use_api = False
                collector(stock, trcode, market, sort, cnt, total_cnt, )
                use_api = True
