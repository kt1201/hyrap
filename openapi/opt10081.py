...
def collector(stock, trcode, company, std_date, cnt, total_cnt, skip, column_list):
    global engine
    global end_date
    ...
    try:
        ...
        if len(df) != 0:
            ...
            df.replace('', np.nan, inplace=True)  # 비어있는 값  np.nan으로 대체
            ...
            for i in range(len(df)):
                ...
                dup_binds = [df.iloc[i]["종목코드"], df.iloc[i]["일자"]]
                ...
                if (len(result) == 0):
                    ...
                    print("{}/{} {}-{}-{} 수집/적재 완료".format(cnt, total_cnt, trcode, company["code"], df.iloc[i]["일자"]))
                    pkey = "{}-{}".format(company["code"], df.iloc[i]["일자"])
                    stock.set_batch_result(trcode, pkey, "SUCCESS", skip)
                else:
                    end_date = 'NO DATA'
        else:
            end_date = 'NO DATA'
            print("{}/{} {}-{}-{} 데이터 없음".format(cnt, total_cnt, trcode, company["code"], std_date))
            pkey = "{}-{}".format(company["code"], std_date)
            stock.set_batch_result(trcode, pkey, "NO DATA", skip)
    except Exception as ex:
        print("{}/{} {}-{}-{} 수집/적재 실패".format(cnt, total_cnt, trcode, company["code"], std_date))
        pkey = "{}-{}".format(company["code"], std_date)
...
def main(stock, trcode):
    ...
    for company in company_list:
        ...
        while(end_date != 'NO DATA'):
            ...
            # 과거 수집/적재 조회
            pkey = "{}-{}".format(company["code"], end_date)
            pkey_column = ["종목코드"]
            skip = stock.past_collect(trcode, pkey, pkey_column)
            if skip == 1:
                print("{}/{} {}-{}-{} 건너뛰기".format(cnt, total_cnt, trcode, company["code"], end_date))
                end_date = "NO DATA"
            elif skip == 2:
                end_date = "NO DATA"
            elif skip == 3:
                end_date = now
                print("{}/{} {}-{} 실패 발견...재적재".format(cnt, total_cnt, trcode, company["code"]))
                stock.delete_company_data(trcode, company)
            else:
                collector(stock, trcode, company, end_date, cnt, total_cnt, skip, column_list)
